"""In-process precinct layer ETL (same behavior as ``scripts/build_precinct_layer.py``)."""
...
def build_precinct_layer_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument(
        ...
        default=None,
        help=(
            "Write JSON manifest (stats + SHA-256 of parquet). "
            "Default: <repo-root>/data/processed/manifests/<out-stem>_etl.json"
        ),
    )
    parser.add_argument(
        "--no-default-manifest",
...
def execute_precinct_layer_etl(args: argparse.Namespace) -> int:
    ...
    if manifest_path is None and not args.no_default_manifest:
        manifest_path = paths.manifest_json(f"{out_parquet.stem}_etl")
    ...
    if args.with_gaps and gap_stats_payload is not None and shell_path is not None:
...
def run_precinct_layer_etl(argv: list[str] | None = None) -> int:
    parser = build_precinct_layer_parser()
    args = parser.parse_args(argv)
    return execute_precinct_layer_etl(args)
