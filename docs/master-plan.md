...
**Deliverables:**
- Implement `build_adjacency` → concrete **`AdjacencyGraph`** (or named tuple: `n_nodes`, `edges` as `list[tuple[int,int]]` or CSR matrix, `order: PrecinctIndexMap`).
- Optional: `save_adjacency` / `load_adjacency` under `data/processed/graph/`.
- Hooks for **manual overrides** (YAML: add/remove edges for bridges/islands) — implement as **immutable “patch”** applied after auto-build, logged.

**OO / structure:**
- **`AdjacencyGraph`**: frozen; holds sparse representation; methods `neighbors(i)`, `degree(i)`.
- **`AdjacencyBuildOptions`**: dataclass (queen/rook, snap tolerance in meters if reprojected).

**Tests:** Known 2×2 grid toy; single island raises or flags; isomorphism check on micro shapefile.

**Docs:** Document queen vs rook choice in [data-model.md](data-model.md).

**Definition of done:** Graph for full Hungary build passes basic connectivity stats (e.g. one giant component unless islands expected).
