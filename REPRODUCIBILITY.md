...
4. **`reports`** — per-county `diagnostics.json` and `partisan_report.json`.
   ...
   - **`partisan_metrics` CLI group (global):** `--partisan-balance` / `--no-partisan-balance`, `--partisan-balance-eps-bloc`, `--partisan-balance-eps-total`, `--partisan-small-values` (`raise` \| `skip_balance` \| `clip`). Party coding overrides: `--reports-party-coding` (reports) and `--policy-figures-party-coding` (`policy_figures`).
